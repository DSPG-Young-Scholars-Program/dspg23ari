...
```{r}
# tart paring down to assignments
...
dataframe[c(2:5),4] <- "Developmental and Broadening"
dataframe[c(2:5),5] <- 2
```
...
pasted <- paste(paragraphs4[5:12], collapse = " " )
finalparagraphs <- c(paragraphs4[1:4],pasted,paragraphs4[13:16])
...
myfunction <- function(input){
  vector <- unlist(str_extract_all(input, pattern = "(?<=\\(\\w\\)\\h).*?(?=\\h\\(\\w\\))"))
  return(vector)
  }
